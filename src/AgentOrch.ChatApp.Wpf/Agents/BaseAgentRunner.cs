...
public class BaseAgentRunner : ObservableObject, INotifyCollectionChanged, INotifyPropertyChanging
{
    ...
    public async Task StartInteractiveChat(AIAgent aIAgent)
    {
        ...
        _ = await aIAgent.CreateSessionAsync();
    ...
    public async Task StartInteractiveChat(Workflow workflow)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                await using StreamingRun run = null!; // await InProcessExecution.StreamAsync(workflow, messages).ConfigureAwait(false);
                _ = await run.TrySendMessageAsync(new TurnToken(true));
