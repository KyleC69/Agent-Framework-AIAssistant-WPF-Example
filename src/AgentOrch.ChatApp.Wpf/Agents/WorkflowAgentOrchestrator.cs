...
using ChatResponseUpdate = Microsoft.Extensions.AI.ChatResponseUpdate;
...
public sealed class WorkflowAgentOrchestrator(IChatClient chatClient, IServiceProvider services)
{
        ...
        IReadOnlyList<object> tools) : IAgentWorkflow
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        if (messageProp?.GetValue(response) is ChatMessage single)
        {
            yield return single;
        }
    }
...
internal static class ToolingSupport
{
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (extType is null)
        {
            throw new InvalidOperationException("Streaming extensions were not found.");
        }
    ...
    public static ChatOptions? CreateChatOptionsWithTools(IReadOnlyList<object> tools)
    {
        ...
        if (toolsProp is null || !toolsProp.CanWrite)
        {
            return options;
        }
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (withOptions is null)
        {
            return chatClient.GetResponseAsync(messages, cancellationToken: cancellationToken);
        }

        object? options = CreateChatOptionsWithTools(tools) ?? TryCreateOptionsWithTools();
        ...
        object? TryCreateOptionsWithTools()
        {
            ...
            if (toolProp is null || !toolProp.CanWrite)
            {
                return null;
            }
